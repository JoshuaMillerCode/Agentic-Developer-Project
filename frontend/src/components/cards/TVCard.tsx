...
import type { TVCard as TVCardType } from "@/lib/types";
...
export function TVCard({
  ...
  const year = card.first_air_date ? new Date(card.first_air_date).getFullYear() : null;

  if (size === "compact") {
    return (
      <Link href={`/tv/${card.id}`} className="group flex-shrink-0 w-28 sm:w-36 rounded-lg overflow-hidden bg-surface-700 transition transform hover:scale-105 hover:ring-2 hover:ring-accent-red/60 block">
        ...
        <div className="p-1.5">
          <p className="font-medium text-sm text-white truncate" title={card.name ?? undefined}>
            {card.name ?? "Untitled"}
          </p>
  ...
  return (
    <Link href={`/tv/${card.id}`} className="group flex-shrink-0 w-40 sm:w-48 rounded-xl overflow-hidden bg-surface-700 transition transform hover:scale-[1.02] hover:shadow-xl hover:shadow-black/30 hover:ring-2 hover:ring-accent-red/50 block">
      <article>
